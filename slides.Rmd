...
# then the colnames() is used to set the same column name in order to bind them together
#
...
# we did not included those graphs in the final presentation since we have the grid graph illustrating both
# there are included here for your reference
plot_bing_words(bing_gym_word_counts, "Most common Positive and Negative words in tweets on #gym")
```
...
gym_sentiment <- gym_sentiment %>% 
  mutate(tweet = "#gym")
#
# using the sentiment_mean function we made to calculate the mean sentiment scores of #homeworkout and #gym
...
kable(sentiments_stats_table, 
      ...
      col.names = c("Tweet", "Mean", "Variance", "Sd", "IQR")) # we set our coloumn names

```

## Statistical Test
```{r visual_inspection_box_plot, echo=FALSE}
...
# p-value < 0.05 
# At .05 significance level, we conclude that the distributions of both populations are not equal, so there is a difference in their medians
# however we should keep in mind the simple random sampling
#
```